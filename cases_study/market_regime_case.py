"""Market-regime case study: SPY vs VIX."""

from __future__ import annotations

import pandas as pd

from src.download_data import download_data, get_date_range, get_ticker_name
...
def run_market_regime_case_study() -> None:
    """Run the SPY vs VIX workflow with the same analysis stack as the finance example."""
    ticker_one = input("First ticker [SPY]: ").strip() or "SPY"
    ticker_two = input("Second ticker [^VIX]: ").strip() or "^VIX"
...
def main() -> None:
    """Entry point for the market-regime case study."""
    run_market_regime_case_study()
