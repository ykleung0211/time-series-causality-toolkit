"""Macro regime case study: inflation vs unemployment."""
...
from src.plotting import plot_dtw_alignment, plot_line_trend, plot_series_comparison
...
def _load_macro_regime_data() -> tuple[pd.Series, pd.Series]:
    """Load a built-in macro series pair without requiring user-supplied files."""
    frame = macrodata.load_pandas().data.copy()
    index = pd.PeriodIndex(frame["year"].astype(int), frame["quarter"].astype(int), freq="Q")
    frame.index = index.to_timestamp(how="end")

    inflation = 400.0 * np.log(frame["cpi"]).diff()
    unemployment = frame["unemp"].astype(float)

    series_one = pd.Series(inflation, index=frame.index, name="inflation_rate").dropna()
    series_two = pd.Series(unemployment, index=frame.index, name="unemployment_rate").dropna()
    aligned_index = series_one.index.intersection(series_two.index)
    return series_one.loc[aligned_index], series_two.loc[aligned_index]


def run_macro_regime_case_study() -> None:
    """Run a built-in macro regime workflow for inflation vs unemployment."""
    name_one = "Inflation"
    name_two = "Unemployment"
    series_one, series_two = _load_macro_regime_data()

    print(f"\nLoaded {len(series_one)} quarterly observations for {name_one} and {name_two} from statsmodels macrodata.")
    ...
    if downsample_enabled:
        ...
        if method == "1":
            ...
            downsample_freq = input("Pandas offset alias (e.g. W, M): ").strip() or "Q"
    ...
    lcc_max_lag = _prompt_int("Maximum lag for lagged cross-correlation (LCC)", 20)
    ...
    stationary_one, meta_one = make_series_stationary(series_one, name_one, alpha=adf_alpha, max_diff_order=max_diff_order)
    stationary_two, meta_two = make_series_stationary(series_two, name_two, alpha=adf_alpha, max_diff_order=max_diff_order)
    ...
    maxlag = _prompt_int("Choose maxlag for Granger causality", 8)
...
def main() -> None:
    """Entry point for the macro regime case study."""
    run_macro_regime_case_study()
